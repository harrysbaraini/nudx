...
import { fileExists, gitInit, writeFile, writeJsonFile } from '../lib/filesystem';
        ...
        task: async (ctx, task) => {
          ...
          return new Listr(
            ctx.sites.map((site: Site) => {
              return {
                title: site.project,
                task: async (ctx: { settings: { sites: { [x: string]: { hash: string; }; }; }; }) => {
                  if (
