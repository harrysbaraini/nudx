...
import { CLI_SRC_PATH } from '../../lib/flags';
...
export default class CompileTemplates extends Command {
  ...
  async run(): Promise<void> {
    ...
    const cliTemplates = await this.getFiles(join(CLI_SRC_PATH, 'templates'));
    const serviceTemplates = await this.getFiles(join(CLI_SRC_PATH, 'services'));
